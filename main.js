	...
	const menuTemplate = [
		{
			...
			submenu: [
				...
				},
				{
					...
					enabled: false,
					click () {
						mainWindow.webContents.executeJavaScript("plse.saveScriptsOnElectron()")
					}
				},
				{
					id: 'exportScript',
					...
					enabled: false,
					click () {
						mainWindow.webContents.executeJavaScript("plse.showScriptExportSettings()")
					}
				},
				{
					id: 'exit',
			...
			submenu: [
				{
					...
					enabled: false,
					click () {
						mainWindow.webContents.executeJavaScript("plse.showGotoRowFilters()")
					}
				}
			]
		},
			...
			submenu: [
				{
					...
					enabled: false,
					click () {
						mainWindow.webContents.executeJavaScript("plse.triggerFocusOnVisibleTableSearchField()")
					}
				}
			]
		},
			...
			submenu: [
				{
					...
					enabled: false,
					click () {
						mainWindow.webContents.executeJavaScript("plse.previewScripts()")
					}
				},
				{
					id: 'analyzeScript',
					...
					enabled: false,
					click () {
						mainWindow.webContents.executeJavaScript("plse.showScriptAnalysisSettings()")
					}
				},
				{
					id: 'configSettings',
					...
					click () {
						mainWindow.webContents.executeJavaScript("plse.showScriptConfigSettings()")
					}
				}
			]
		},
			...
			submenu: [
				{
					...
					accelerator: 'F1',
					click () {
						mainWindow.webContents.executeJavaScript("plse.showInstructions()")
					}
				},
				{
					id: 'about',
...
function registerMainTabsShortcuts(){
	globalShortcut.register('CmdOrCtrl+PageDown', () => {
		mainWindow.webContents.executeJavaScript("plse.switchMainTab(true)")
	})
	globalShortcut.register('CmdOrCtrl+PageUp', () => {
		mainWindow.webContents.executeJavaScript("plse.switchMainTab(false)")
	})
}
...
function registerScriptsTabsShortcuts(totalScriptsTabs){
	for(let i=1; i<=totalScriptsTabs; i++){
		globalShortcut.register('CmdOrCtrl+'+i, () => {
			mainWindow.webContents.executeJavaScript("plse.triggerClickOnScriptTabByNumber(" + i + ")")
		})
	}
}
